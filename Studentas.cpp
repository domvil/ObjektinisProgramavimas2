...
double Studentas::skaiciuotiGalutini(){
     return 0.4*c_med_vid+0.6*(double)c_egz;
}

double Studentas::avg(){
    int n = getNdSize();
    if(n>0){
        return accumulate(c_nd.begin(), c_nd.end(),0.0)/(double)n;
    }
    return 0;
}
...
double Studentas::median(){
    ...
    sortNd();
    if(n%2!=0){
        return (double)c_nd[n/2];
    }else{
        return (double)(c_nd[(n-1)/2]+c_nd[n/2])/2.0;
    }
}

void Studentas::print() {
  printf("%-12s %-12s %-16.2lf \n", c_vardas.c_str(),c_vardas.c_str(), c_rez);
}
